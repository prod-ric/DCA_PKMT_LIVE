...
class MarketState:
    ...
    last_entry_time: Dict[str, str] = field(default_factory=dict)
    ...
    def to_dict(self) -> dict:
        d = asdict(self)
        d["positions"] = {
            k: v.to_dict() if isinstance(v, Position) else v
...
class PositionTracker:
    ...
    def __init__(
        ...
        self.price_history: Dict[str, Dict[str, List]] = {}
    ...
    def init_market(
        ...
        """Initialise or get a market state."""
        if market_id not in self.markets:
            ...
            logger.info(
                f"Initialised market {name or market_id[:20]}... alloc=${allocation:.2f}"
            )
        return self.markets[market_id]

    def record_trade(self, trade: Trade):
        """Record a trade and append to JSONL log file."""
        self.trades.append(trade)
        try:
            dirname = os.path.dirname(self.trade_log_file)
            if dirname:
                os.makedirs(dirname, exist_ok=True)
    ...
    def update_price(self, market_id: str, asset_id: str, price: float, ts: datetime):
        ...
        cutoff = (ts - timedelta(seconds=300)).isoformat()
        self.price_history[market_id][asset_id] = [
        ...
        long_cutoff = (ts - timedelta(minutes=60)).isoformat()
        self.long_price_history[market_id][asset_id] = [
            (t, p)
            for t, p in self.long_price_history[market_id][asset_id]
            if t >= long_cutoff
        ]

    def get_price(self, market_id: str, asset_id: str) -> float:
        return self.price_cache.get(f"{market_id}:{asset_id}", 0.0)
    ...
    def get_total_pnl(self) -> float:
        """Total realised + unrealised PnL across all markets."""
    ...
    def print_status(self):
        ...
        for mid, ms in self.markets.items():
            ...
            if ms.panic_flip_triggered:
                status += " [PANIC]"
            print(
                f"  {ms.name or mid[:25]:25s} | pnl=${ms.pnl:+.2f} | pos={open_pos}{status}"
            )
        print(f"{'=' * 60}\n")
